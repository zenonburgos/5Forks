...
export class EditCicloComponent implements OnInit{
  ...
  public h_inicio = { hour: new Date().getHours(), minute: new Date().getMinutes() };
  public h_fin = { hour: new Date().getHours(), minute: new Date().getMinutes() };
  ...
  agregar_ciclo(){
    ...
    if (!this.salon.salon) {
      ...
      $.notify('Ingrese la hora de finalización.', { 
        type: 'danger',
        spacing: 10,                    
        timer: 2000,
        placement: {
      ...
      $.notify('Debe seleccionar al menos un día.', { 
        type: 'danger',
        spacing: 10,                    
        timer: 2000,
        placement: {
  ...
  eliminar_salon(idx:any){
    ...
    this._cursoService.eliminar_salon_ciclo_admin(idx,this.token).subscribe(
      response=>{
        $.notify('Se ha eliminado el salón del ciclo.', { 
          type: 'success',
          spacing: 10,                    
          timer: 2000,
          placement: {
  ...
  actualizar(){
    ...
    if (!this.ciclo.nivel) {
      ...
      $.notify('Seleccione una sede del ciclo.', { 
        type: 'danger',
        spacing: 10,                    
        timer: 2000,
        placement: {
      ...
      $.notify('Seleccione la fecha de inicio de clases.', { 
        type: 'danger',
        spacing: 10,                    
        timer: 2000,
        placement: {
      ...
      $.notify('Seleccione la fecha fin de clases.', { 
        type: 'danger',
        spacing: 10,                    
        timer: 2000,
        placement: {
  ...
  vincular_docente_salon(){
    ...
    if (!this.docente_salon.colaborador) {
      ...
      this._cursoService.agregar_docente_salon_admin(this.docente_salon,this.token).subscribe(
        response=>{
          $.notify('Se agregó la vinculación del docente.', { 
            type: 'success',
            spacing: 10,                    
            timer: 2000,
            placement: {
  ...
  eliminar_docente_salon(id:any){
    ...
    this._cursoService.eliminar_docente_salon_admin(id,this.token).subscribe(
      response=>{
        $.notify('Se ha removido el docente del salón.', { 
          type: 'success',
          spacing: 10,                    
          timer: 2000,
          placement: {
