...
export class CreateMatriculaComponent implements OnInit{
  ...
  ngOnInit(): void {
    //this.init_clientes(); //Para utilizar el filtrado de clientes esto no debe inicializarse aquí
  ...
  aplicar_descuento(){
    if(this.descuento_tipo == 'Precio fijo'){
      ...
      if(this.descuento_valor < 100){
        ...
        $.notify('El porcentaje de descuento no puede ser menor que 100%.', { 
          type: 'danger',
          spacing: 10,                    
          timer: 2000,
          placement: {
  ...
  matricular(){
    ...
    if(!this.matricula.cliente){
      ...
      $.notify('Ingrese el monto de la matrícula por favor.', { 
        type: 'danger',
        spacing: 10,                    
        timer: 2000,
        placement: {
      ...
      $.notify('Ingrese un monto válido en la matrícula.', { 
        type: 'danger',
        spacing: 10,                    
        timer: 2000,
        placement: {
      ...
      $.notify('Debe al menos ingresar un ciclo.', { 
        type: 'danger',
        spacing: 10,                    
        timer: 2000,
        placement: {
      ...
      this.matricula.detalles = this.detalles //Agregamos array detalles al ya existente matricula
      this.btn_load = true;
      this._matriculaService.generar_matricula_admin(this.matricula, this.token).subscribe(
        response=>{
          $.notify('La matrícula fue completada.', { 
            type: 'success',
            spacing: 10,                    
            timer: 2000,
            placement: {
